...
export class GridUtilService {
  ...
  calculateCustomSummary(o, tt): void {
    const arr = tt.summaryList?.map(el => {
      el[o.name] = el[o.name] ? el[o.name] : 0; // null 항목 초기화
      return el;
    });
  ...
  getDecade(): any[] {
    const years = [];
    const year = Number(this.getToday().substr(0, 4));
    const startYear = Math.floor(year / 10) * 10;
    for (let i = startYear; i < startYear + 10; i++) {
      years.push({cd: String(i), nm: String(i)});
    }
  ...
  getJanToDec(): any[] {
    ...
    for(let i = 1; i <= 12; i++) {
      ...
      months.push({cd: String(mon), nm: String(mon)});
    }
    return months;
    // [{cd: '01', nm: '01' + utilService.convert('sales.mon')},
    //   {cd: '02', nm: '02' + utilService.convert('sales.mon')},
    //   {cd: '03', nm: '03' + utilService.convert('sales.mon')},
    //   {cd: '04', nm: '04' + utilService.convert('sales.mon')},
    //   {cd: '05', nm: '05' + this.utilService.convert('sales.mon')},
    //   {cd: '06', nm: '06' + this.utilService.convert('sales.mon')},
    //   {cd: '07', nm: '07' + this.utilService.convert('sales.mon')},
    //   {cd: '08', nm: '08' + this.utilService.convert('sales.mon')},
    //   {cd: '09', nm: '09' + this.utilService.convert('sales.mon')},
    //   {cd: '10', nm: '10' + this.utilService.convert('sales.mon')},
    //   {cd: '11', nm: '11' + this.utilService.convert('sales.mon')},
    //   {cd: '12', nm: '12' + this.utilService.convert('sales.mon')}];
  }
}
