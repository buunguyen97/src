...
import {InvTagWhLocationLotService, InvTagWhLocationLotVO} from './inv-tag-wh-location-lot.service';

@Component({
  selector: 'app-inv-tag-wh-location-lot',
  templateUrl: './inv-tag-wh-location-lot.component.html',
  styleUrls: ['./inv-tag-wh-location-lot.component.scss']
})
export class InvTagWhLocationLotComponent implements OnInit, AfterViewInit {
  ...
  // ***** main ***** //
  // Form
  mainFormData: InvTagWhLocationLotVO = {} as InvTagWhLocationLotVO;
  dsLocGroup = [];
  dsLocation = [];
  dsLocationId = [];
  dsDamageFlg = [];
  ...
  constructor(
    ...
    private service: InvTagWhLocationLotService,
  ...
  saveState = (state) => {
    ...
    localStorage.setItem('wi_invTagwhLocationLot', JSON.stringify(state));
  }

  // 그리드 상태 로드
  loadState = () => {
    return new Promise((resolve, reject) => {
      const data = localStorage.getItem('wi_invTagwhLocationLot');
      if (data) {
  ...
  async onViewReport(): Promise<void> {
    const locationId = this.mainForm.instance.getEditor('locationId').option('value');
    const itemId = this.mainForm.instance.getEditor('itemId').option('value');

    if (!locationId) {
      const msg = this.utilService.convert1('invtagwh_locationvalidation', '조회조건의 로케이션을 선택하세요.');
      this.utilService.notify_error(msg);
      return;
    }

    if (!itemId) {
      const msg = this.utilService.convert1('invtagwh_itemvalidation', '조회조건의 품목을 선택하세요.');
      this.utilService.notify_error(msg);
      return;
    }

    const reportFile = 'file=finishedInvList.jrf';
    const reportOption = [
        {
          dataSet: 'DataSet0',
          node: 'data',
          path: '/report-service/report/findFinishedInvList',
          apiParam: {
            tenant: this.G_TENANT,
            warehouseId: this.mainFormData.warehouseId,
            ownerId: this.mainFormData.ownerId,
            // locationId: this.mainFormData.locationId,
            // itemAdminId: this.mainFormData.itemAdminId,
            // itemId: this.mainFormData.itemId
            locationId: (this.mainFormData.locationId === undefined) ? null : this.mainFormData.locationId,
            itemAdminId: (this.mainFormData.itemAdminId === undefined) ? null : this.mainFormData.itemAdminId,
            itemId: (this.mainFormData.itemId === undefined) ? null : this.mainFormData.itemId
          }
        }
      ]
    ;
