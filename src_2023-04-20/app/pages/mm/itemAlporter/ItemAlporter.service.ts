...
export class ItemAlporterService {


  // 기본 URL 선언
  httpUrl = `${APPCONSTANTS.BASE_URL_WM}/master-service/itemalporter`;
  ...
  async findItemFull(data: {}): Promise<ApiResult<ItemVO>> {
    const baseUrl = `${this.httpUrl}/findItemFull`;
    try {
      const result = await this.http.post<ApiResult<ItemVO>>(baseUrl, data).toPromise();
      return result;
    } catch (e) {
