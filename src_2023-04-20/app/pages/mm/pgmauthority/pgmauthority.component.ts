...
export class PgmauthorityComponent implements OnInit, AfterViewInit {
  ...
  async chooseVisibleY(): Promise<void> {
    ...
    for (const checked of checkedList) {
      if (checked.appId > -1) {
        ...
        if (findItem) {
          findItem.data.authSearch = RcvCommonUtils.FLAG_TRUE;
        } else {
          if (checked.uid > -1) {
            this.changes2.push({type: 'update', key: checked.keyExpr, data: {authSearch: RcvCommonUtils.FLAG_TRUE}});
          } else {
            this.changes2.push({type: 'insert', key: checked.keyExpr, data: {authSearch: RcvCommonUtils.FLAG_TRUE}});
          }
        }
      }
    }
  ...
  async chooseVisibleN(): Promise<void> {
    ...
    for (const checked of checkedList) {
      if (checked.appId > -1) {
        const findItem = this.changes2.find(el => el.key === checked.keyExpr);
        if (findItem) {
          findItem.data.authSearch = RcvCommonUtils.FLAG_FALSE;
        } else {
          if (checked.uid > -1) {
            this.changes2.push({type: 'update', key: checked.keyExpr, data: {authSearch: RcvCommonUtils.FLAG_FALSE}});
          } else {
            this.changes2.push({type: 'insert', key: checked.keyExpr, data: {authSearch: RcvCommonUtils.FLAG_FALSE}});
          }
        }
      }
    }
  ...
  async onSearch(gridNm: string): Promise<void> {
    ...
    if (data.isValid) {
      ...
      let result;
      if (gridNm === 'popup') {
        result = await this.service.getUsers({tenant: '1000'});
      } else {
        result = await this.service.getUsers(this.mainFormData);
      }
      ...
      if (!result.success) {
        ...
        await this.utilService.notify_success('search success');
  ...
  async saveAdmin(): Promise<void> {
    const confirmMsg = this.utilService.convert('confirmSave', this.utilService.convert('관리자저장'));
