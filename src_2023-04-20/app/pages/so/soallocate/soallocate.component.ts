...
import {AllocateSaveVO, SoallocateService} from './soallocate.service';
import {GridUtilService} from '../../../shared/services/grid-util.service';
import {SoCommonUtils} from '../soCommonUtils';
import {SoVO} from "../so/so.service";

@Component({
  selector: 'app-soallocate',
  templateUrl: './soallocate.component.html',
  styleUrls: ['./soallocate.component.scss']
})
export class SoallocateComponent implements OnInit, AfterViewInit {
  ...
  saveFormData = {} as AllocateSaveVO;
  ...
  constructor(public utilService: CommonUtilService,
              ...
              private service: SoallocateService
  ) {
  ...
  ngAfterViewInit(): void {
    ...
    this.initData(this.saveForm, 'save');
  }
  ...
  async onExcute(): Promise<void> {
    ...
    const msgStr = this.utilService.convert('so_soallocate_title');
    ...
    if (data.isValid) {
      ...
      if (selectRowKeys.length > 0) {
        ...
        const allocateSaveVO = {} as AllocateSaveVO;
        allocateSaveVO.list = selectRowKeys;
        allocateSaveVO.tenant = this.G_TENANT;
        allocateSaveVO.pickType = this.saveFormData.pickType;
        allocateSaveVO.pickGroup = this.saveFormData.pickGroup;
        allocateSaveVO.pickAllocGroup = this.saveFormData.pickAllocGroup;
        allocateSaveVO.delivSchDate = this.saveFormData.delivSchDate;
        allocateSaveVO.remarks = this.saveFormData.remarks;

        const result = await this.service.save(allocateSaveVO);
  ...
  onSelectionChanged(e): void {
    ...
    this.mainGrid.instance.byKey(selectedRowKey).then(val => {
      ...
      if (sts !== SoCommonUtils.STS_ACCEPTED) {
    ...
    dataList.forEach(el => {
      ...
      if (el.sts !== SoCommonUtils.STS_ACCEPTED) {
