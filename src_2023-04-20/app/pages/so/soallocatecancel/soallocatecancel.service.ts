...
export class SoallocatecancelService {

  // 기본 URL 선언
  httpUrl = `${APPCONSTANTS.BASE_URL_WM}/release-service/allocateCancel`;
  ...
  async get(searchData: {}): Promise<ApiResult<PickVO[]>> {
    // Api 설정
    const baseUrl = `${this.httpUrl}/findAllocateCancel`;
  ...
  async save(data: any[]): Promise<ApiResult<void>> {
    // Api 설정
    const baseUrl = `${this.httpUrl}/saveAllocateCancel`;
