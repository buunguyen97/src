...
import {SoallocatecancelService} from './soallocatecancel.service';
...
@Component({
  selector: 'app-soallocatecancel',
  templateUrl: './soallocatecancel.component.html',
  styleUrls: ['./soallocatecancel.component.scss']
})
export class SoallocatecancelComponent implements OnInit, AfterViewInit {
  ...
  GRID_STATE_KEY = 'so_soallocatecancel';
  ...
  constructor(public utilService: CommonUtilService,
              ...
              private service: SoallocatecancelService
  ) {
  ...
  async onExcute(): Promise<void> {
    const msgStr = this.utilService.convert('so_soallocatecancel_title');
  ...
  initData(form): void {
    ...
    form.formData = {
      ...
      sts: SoCommonUtils.STS_ALLOCATED,
  ...
  onSelectionChanged(e): void {
    ...
    this.mainGrid.instance.byKey(selectedRowKey).then(val => {
      ...
      if (sts !== SoCommonUtils.STS_ALLOCATING && sts !== SoCommonUtils.STS_ALLOCATED) {
    ...
    dataList.forEach(el => {
      ...
      if (el.sts !== SoCommonUtils.STS_ALLOCATING && el.sts !== SoCommonUtils.STS_ALLOCATED) {
