...
import {SopickService} from './sopick.service';
...
@Component({
  selector: 'app-sopick',
  templateUrl: './sopick.component.html',
  styleUrls: ['./sopick.component.scss']
})
export class SopickComponent implements OnInit, AfterViewInit {
  ...
  stsList = [SoCommonUtils.STS_ALLOCATED, SoCommonUtils.STS_PICKING];

  GRID_STATE_KEY = 'so_sopick';
  ...
  async onExcute(): Promise<void> {
    const msgStr = this.utilService.convert('so_sopick_titleBtn');
    ...
    if (selectList.length > 0) {
      ...
      for (const selectRow of selectList) {
        ...
        const pickedQty1 = this.mainGrid.instance.cellValue(rowIdx, 'pickedQty1');
        const expectQty1 = this.mainGrid.instance.cellValue(rowIdx, 'expectQty1');

        if (!pickedQty1) {
          this.utilService.notify_error(this.utilService.convert('so_valid_qtygt', this.utilService.convert('so_so_pickedQty1'), '0'));
          this.setFocusRow(rowIdx);
          check = true;
          break;
        } else {

          if (expectQty1 < pickedQty1) {
            this.utilService.notify_error(this.utilService.convert('so_valid_qtylt', this.utilService.convert('so_so_pickedQty1'),
              this.utilService.convert('so_so_expectQty1')));
            this.setFocusRow(rowIdx);
            check = true;
            break;
          }
          selectRow.pickedQty1 = pickedQty1;
        }
      }
      ...
      const result = await this.service.save(selectList);
  ...
  onReset(): void {
    ...
    this.initData(this.mainForm);
    this.stsList = [SoCommonUtils.STS_ALLOCATED, SoCommonUtils.STS_PICKING];
  }
  ...
  onSelectionChanged(e): void {
    ...
    this.mainGrid.instance.byKey(selectedRowKey).then(val => {
      ...
      if (sts !== SoCommonUtils.STS_ALLOCATED && sts !== SoCommonUtils.STS_PICKING) {
    ...
    dataList.forEach(el => {
      ...
      if (el.sts !== SoCommonUtils.STS_ALLOCATED && el.sts !== SoCommonUtils.STS_PICKING) {
