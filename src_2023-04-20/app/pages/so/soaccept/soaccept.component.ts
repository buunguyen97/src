...
import {SoacceptService} from './soaccept.service';
import {GridUtilService} from '../../../shared/services/grid-util.service';
import {SoCommonUtils} from '../soCommonUtils';
import {SoVO} from '../so/so.service';

@Component({
  selector: 'app-soaccept',
  templateUrl: './soaccept.component.html',
  styleUrls: ['./soaccept.component.scss']
})
export class SoacceptComponent implements OnInit, AfterViewInit {
  ...
  constructor(public utilService: CommonUtilService,
              ...
              private service: SoacceptService
  ) {
  ...
  async onExcute(): Promise<void> {
    const msgStr = this.utilService.convert('so_soaccept_title');
    const selectRowKeys = this.mainGrid.selectedRowKeys;

    if (selectRowKeys.length > 0) {
      // let check = false;
      //
      // for (const rowKey of selectRowKeys) {
      //   const rowIdx = this.mainGrid.instance.getRowIndexByKey(rowKey);
      //   const sts = this.mainGrid.instance.cellValue(rowIdx, 'sts');
      //
      //   if (sts !== '100') {
      //     this.utilService.notify_error(this.utilService.convert('so_msg_ableSts', this.utilService.convert('COMMONCODE.SOSTATUS.100')));
  ...
  onSelectionChanged(e): void {
    ...
    dataList.forEach(el => {
      ...
      if (el.sts !== SoCommonUtils.STS_IDLE) {
