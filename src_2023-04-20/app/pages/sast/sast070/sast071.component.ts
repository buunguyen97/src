...
@Component({
  selector: 'app-sast071',
  templateUrl: './sast071.component.html',
  styleUrls: ['./sast071.component.scss']
})
export class Sast071Component implements OnInit, AfterViewInit {
  ...
  async procAdjust(): Promise<void> {
    ...
    if (popData.isValid) {
      try {
        ...
        const saveContent = this.popupData as Sast070VO;
        // const aaa = this.bizService.collectGridData(this.changes, this.popupGrid, this.G_TENANT);
        const detailList = this.popupGrid.instance.getDataSource().items();

        this.popupDataSource.store().load().then(async (res) => {
          if (res !== undefined && res.length > 0) {
            ...
            saveContent.itemList = res;
            console.log(saveContent);

            result = await this.service.procAdjust(JSON.stringify(saveContent));
            if (!result.success) {
              this.utilService.notify_error(result.msg);
              return;
            } else {
