...
import {Sacl031Service, Sacl031VO} from './sacl031.service';

@Component({
  selector: 'app-sacl031',
  templateUrl: './sacl031.component.html',
  styleUrls: ['./sacl031.component.scss']
})
export class Sacl031Component implements OnInit, AfterViewInit {

  constructor(public utilService: CommonUtilService,
              private service: Sacl031Service,
    ...
    this.setInpAmt = this.setInpAmt.bind(this);
  }
  ...
  mainFormData: Sacl031VO = {} as Sacl031VO;
  ...
  async onSave(): Promise<void> {
    try {
      ...
      const saveContent = this.saveData as Sacl031VO;
      ...
      if (detailList.length === 0) {
        ...
        for ( const items of detailList ) {
          if (items.pur_cls_yn_dp === 'N') {
            // 계산서정보는 마감 후에 등록이 가능합니다.
            const msg = this.utilService.convert('Sales.PurClsSave'
                                                , this.utilService.convert1('sales.invoice_info', '계산서정보', 'Invoice Info')
                                                , this.utilService.convert1('sales.Possibility', '가능', 'Possibility')
                                                );
  ...
  // 매입전표 부대비용 저장
  async onPurAddSave(): Promise<void> {
    ...
    try {
      ...
      if (detailList.length === 0) {
        ...
        for ( const items of detailList ) {
          if (items.pur_cls_yn_dp === 'Y') {
            // 부대비용은 마감 후에 등록이 불가합니다.
            const msg = this.utilService.convert('Sales.PurClsSave'
                                                , this.utilService.convert1('sales.add_price', '부대비용', '부대비용')
                                                , this.utilService.convert1('sales.Impossibility', '불가능', 'Impossibility')
                                                );
      ...
      const confirmMsg = this.utilService.convert('confirmSave', this.utilService.convert1('sales.add_price', '부대비용', '부대비용'));
      if (!await this.utilService.confirm(confirmMsg)) {
        return;
      }
      ...
      result = await this.service.purAddSave(saveContent);

      if (!result.success) {
        if (result.msg === 'Sales.ProcCallFail') {
          const msg = this.utilService.convert1('Sales.ProcCallFail', '프로시저 호출에 실패하였습니다.', 'Procedure call failed.');
  ...
  onEditorPreparing(e): void {
    ...
    if (e.dataField === 'add_price1' && e.parentType === 'dataRow'){
      e.editorOptions.disabled = (!e.row.data.pur_cls_yn && e.row.data.pur_cls_yn_dp === 'N' && e.row.data.ord_gb === '1') ? false : true;
    }

    if (e.dataField === 'add_price2' && e.parentType === 'dataRow'){
      e.editorOptions.disabled = (!e.row.data.pur_cls_yn && e.row.data.pur_cls_yn_dp === 'N' && e.row.data.ord_gb === '1') ? false : true;
    }

    if (e.dataField === 'add_price3' && e.parentType === 'dataRow'){
      e.editorOptions.disabled = (!e.row.data.pur_cls_yn && e.row.data.pur_cls_yn_dp === 'N' && e.row.data.ord_gb === '1') ? false : true;
    }

    if (e.dataField === 'add_price4' && e.parentType === 'dataRow'){
      e.editorOptions.disabled = (!e.row.data.pur_cls_yn && e.row.data.pur_cls_yn_dp === 'N' && e.row.data.ord_gb === '1') ? false : true;
    }

    if (e.dataField === 'add_price5' && e.parentType === 'dataRow'){
      e.editorOptions.disabled = (!e.row.data.pur_cls_yn && e.row.data.pur_cls_yn_dp === 'N' && e.row.data.ord_gb === '1') ? false : true;
    }
