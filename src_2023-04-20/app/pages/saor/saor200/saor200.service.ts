...
export class Saor200Service {

  // 기본 URL 선언
  httpUrl = `${APPCONSTANTS.BASE_URL_SL}/sales-service/saor200`;

  // http 객체 Injection
  constructor(private http: JHttpService) { }

     // 메인 조회
   async mainList(searchData: {}): Promise<ApiResult<Saor200VO[]>> {
     // 조회 Api 설정
     const baseUrl = `${this.httpUrl}/mainList`;

     try {
       // Post 방식으로 조회
       const result = await this.http.post<ApiResult<Saor200VO[]>>(baseUrl, searchData).toPromise();
       return result;
     } catch (e) {
       return {
         success: false,
   ...
   // 제품 목록
   async detailList(data: {}): Promise<ApiResult<Saor200VO>> {
       // 조회 Api
  ...
  async contNoInfo(data: {}): Promise<ApiResult<Saor200VO>> {
       // 조회 Api
   ...
   // 수정함수
   async mainSave(data: {}): Promise<ApiResult<Saor200VO>> {
    // 조회 Api 설정
  ...
  // 저장함수
  async mainInsert(data: {}): Promise<ApiResult<Saor200VO>> {
    // 조회 Api 설정
  ...
  // 삭제함수
  async mainDelete(data: {}): Promise<ApiResult<Saor200VO>> {
  ...
  async gridInfo2Search(data: {}): Promise<ApiResult<any>> {
   ...
   // I/F함수
   async sendApi(data: {}): Promise<ApiResult<Saor200VO>> {
    // 조회 Api 설정
