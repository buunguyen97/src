...
@Component({
  selector: 'app-saor021',
  templateUrl: './saor021.component.html',
  styleUrls: ['./saor021.component.scss']
})
export class Saor021Component implements OnInit, AfterViewInit {
...
// 주문일자 변경시 - 계약번호 변경
  ...
  async popupSaveClick(e): Promise<void> {
    ...
    if (popData.isValid) {
      try {
        ...
        if (!this.rentEndDt.value) {
          const msg = '렌탈 종료일자를 입력하세요.';
          this.utilService.notify_error(msg);
          return;
        }
        this.popupFormData.rent_end_dt = String(this.rentEndDt.value).replace(/-/gi, '');
    ...
    this.bizService.getContNo(tenant, contGb, exptCd, imptCd, date).subscribe(result => {
      if (result.data) {
        ...
        if (this.popupMode === 'Edit' && !!this.popupFormData.cont_no) {
          console.log('한거야 안한거야', this.popupFormData.cont_no);
  ...
  popupShown(e): void {
    ...
    this.fetchContNo(this.G_TENANT, '2', this.popupFormData.expt_cd, this.popupFormData.impt_cd, this.popupFormData.ord_dt);
