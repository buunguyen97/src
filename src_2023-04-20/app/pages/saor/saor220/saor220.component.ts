...
import { Saor220Service, Saor220VO } from './saor220.service';

@Component({
  selector: 'app-saor220',
  templateUrl: './saor220.component.html',
  styleUrls: ['./saor220.component.scss']
})
export class Saor220Component implements OnInit {

   constructor(
    public utilService: CommonUtilService,
    public gridUtil: GridUtilService,
    private service: Saor220Service,
  ...
  mainFormData: Saor220VO = {} as Saor220VO;
  ...
  popupVisible2 = false;
  popupMode = 'Add';
  popupFormData: Saor220VO;
  popupFormData2: Saor220VO;
  subFormData: Saor220VO;
  firstPopupData = "";
  ...
  async onItemSearch2(): Promise<void> {
    ...
    const result = await this.service.gridInfo2(this.popupFormData.ord_no);
    if (!result.success) {
      return;
    }else{
  ...
  async popupSaveClick(e): Promise<void> {
    ...
    if (popData.isValid) {
      try {
        ...
        const saveContent = this.popupFormData as Saor220VO;
  ...
  onSelectionChangedExptCd(e): void {
    ...
    if( e ) {
      console.log(e)
      if( e.value == undefined){
        this.bizService.getOrdNoC(this.G_TENANT, this.popupFormData.expt_cd).subscribe(result => {
  ...
  async popupDeleteClick(e): Promise<void> {
    try {
      ...
      const deleteContent = this.popupFormData as Saor220VO;
