...
import {formData, Saor241Service} from './saor241.service';

@Component({
  selector: 'app-saor241',
  templateUrl: './saor241.component.html',
  styleUrls: ['./saor241.component.scss']
})
export class Saor241Component implements OnInit, AfterViewInit {

  constructor(public utilService: CommonUtilService,
              private service: Saor241Service,
    ...
    this.returnCancel = this.returnCancel.bind(this);
  ...
  async returnCancel(e): Promise<void> {

    try {
      const confirmMsg = this.utilService.convert1('Execute_returnCancel', '반품확정을 취소하시겠습니까?');
      if (!await this.utilService.confirm(confirmMsg)) {
        return;
      }
      const result = await this.service.returnCancel(this.popupFormData);
